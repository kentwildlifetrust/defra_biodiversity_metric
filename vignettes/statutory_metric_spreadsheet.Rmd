...
#O12 is required action to meet trading rules - assume this always gives a satisfactory result
#X12 is distinctiveness_score
#I12 is baseline_distinctiveness_score
#R12 is enhanced_habitat_label
...
#start with all possible combinations of post_habitat_type, post_condition, base_distinctiveness, base_condition
enhancement <- expand.grid(
  post_habitat_type_id = habitat_types$habitat_type_id,
  post_condition_id = conditions$condition_id,
  base_distinctiveness_id = distinctivenesses$distinctiveness_id,
...
#filter down the distinctiveness and condition combinations to allowed combinations
distinctiveness_conditions <- habitat_conditions %>%
  dplyr::left_join(habitat_types, by = "habitat_type_id") %>%
  dplyr::group_by(condition_id, distinctiveness_id) %>%
  dplyr::summarise() %>%
  dplyr::mutate(is_allowed = T)

enhancement <- enhancement %>%
  dplyr::left_join(distinctiveness_conditions,
...
enhancement <- dplyr::bind_rows(enhancement_lower_distinctiveness, enhancement_same_distinctiveness) %>%
  dplyr::select(base_distinctiveness_id, base_condition_id, post_habitat_type_id, post_condition_id, years_to_target_id)
...
stopifnot(!any(duplicated(dplyr::select(enhancement, post_habitat_type_id, post_condition_id, base_distinctiveness_id, base_condition_id))))
...
max_rows <- 246
#baseline data
habitat_parcels <- data.frame(
  habitat_label_id = sample(habitat_labels %>%
                              dplyr::filter(is_baseline) %>%
                              dplyr::pull(habitat_label_id), max_rows, replace = T),
  enhanced_area_ha = runif(max_rows, 0, 100),
  offsite_reference = 1
)

habitat_parcels <- habitat_parcels %>%
  dplyr::left_join(habitat_labels, by = "habitat_label_id") %>%
  dplyr::left_join(habitat_types, by = "habitat_type_id") %>%
  dplyr::left_join(distinctivenesses, by = "distinctiveness_id") %>%
  dplyr::mutate(condition_id = sapply(habitat_type_id, function(x) {
  ...
  dplyr::left_join(conditions, by = "condition_id") %>%
  dplyr::select(offsite_reference,
                base_habitat_type_id = habitat_type_id,
                base_habitat_label = habitat_label,
                enhanced_area_ha,
                base_distinctiveness_id = distinctiveness_id,
                base_distinctiveness_score = distinctiveness_score,
  ...
  dplyr::mutate(
    ...
    post_habitat_type_id = mapply(FUN = function(d, c) {
      matches <- enhancement %>%
        dplyr::filter(base_distinctiveness_id == d &
                      base_condition_id == c)
      if (nrow(matches) == 1) return(matches$post_habitat_type_id)
      if (nrow(matches) == 0) return(NA)
      return(sample(matches$post_habitat_type_id, 1))
    }, base_distinctiveness_id, base_condition_id),
    
  ) %>%
    dplyr::mutate(
    post_condition_id = mapply(FUN = function(h, d, c) {
      matches <- enhancement %>%
        dplyr::filter(base_distinctiveness_id == d &
                      base_condition_id == c &
                      post_habitat_type_id == h)
      if (nrow(matches) == 1) return(matches$post_condition_id)
      if (nrow(matches) == 0) return(NA)
      return(sample(matches$post_condition_id, 1))
    }, post_habitat_type_id, base_distinctiveness_id, base_condition_id)
  ) %>%
  
  dplyr::left_join(enhancement,
                   by = c("post_habitat_type_id" = "post_habitat_type_id",
                          "base_distinctiveness_id" = "base_distinctiveness_id",
                          "base_condition_id" = "base_condition_id",
                          "post_condition_id" = "post_condition_id")) %>%
  ...
  dplyr::left_join(habitat_labels %>%
                     ...
                     dplyr::left_join(habitat_types, by = "habitat_type_id") %>%
                     dplyr::select(post_habitat_type_id = habitat_type_id,
                                   post_habitat_label = habitat_label),
                   by = "post_habitat_type_id") %>%
  dplyr::mutate(
